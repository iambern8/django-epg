#!/usr/bin/env python3
"""
EPG Fetcher fuer Strong8k IPTV (Xtream Codes)
Holt XMLTV-Daten vom Provider und speichert sie als epg_strong8k.xml
Zugangsdaten werden ueber Umgebungsvariablen uebergeben (GitHub Secrets).
"""
...
def validate_env():
    """Prueft ob alle noetigen Umgebungsvariablen gesetzt sind."""
    missing = []
    if not XTREAM_HOST:
...
def count_channels_and_programmes(xml_text):
    """Zaehlt Kanaele und Programme im XMLTV."""
...
def main():
    ...
    print(f"Kanaele: {channels}")
    print(f"Programme: {programmes}")
    print("=" * 50)

    # Coverage-Check: Kanaele ohne Programme auflisten
